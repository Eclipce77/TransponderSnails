...
/**
 * CLIENT-SIDE manager for Baby Black Transponder Snail call states
 * Tracks state synced from server via packets
 *
 * IMPORTANT: Predicate values MUST match the model JSON file thresholds:
 * - baby_black_transponder_snail.json uses: 0.0 (idle), 0.1 (sound), 0.2 (call), 0.3 (active)
 */
public class BabyBlackSnailCallStateManager {

    private static final BabyBlackSnailCallStateManager INSTANCE = new BabyBlackSnailCallStateManager();
    ...
    /**
     * Call states with predicate values matching baby_black_transponder_snail.json model thresholds
    ...
    private BabyBlackSnailCallStateManager() {}

    public static BabyBlackSnailCallStateManager getInstance() {
        return INSTANCE;
    }
    ...
    public void setState(UUID playerId, CallState state) {
        System.out.println("[BABY-BLACK-STATE-MGR] setState: player=" + playerId.toString().substring(0, 8) +
